...
def create_expense_claim(prompt, pdf_path):
	frappe.throw(str(frappe.utils.get_url()))
	if "private" in pdf_path:
		site_path = frappe.get_site_path(pdf_path)
		full_path = os.path.join("/home/", "private", pdf_path)
	else:
		site_path = frappe.get_site_path(pdf_path)
		full_path = os.path.join(site_path, "public", pdf_path)

	frappe.throw(str(site_path))
